...
from LSP.plugin.core.diagnostics import (DiagnosticsStorage, DiagnosticsWalker, DiagnosticsCursor, CURSOR_FORWARD,
                                         CURSOR_BACKWARD)
...
def make_update(diagnostics: 'List[dict]') -> dict:
    return {'uri': 'file:///test.py', 'diagnostics': diagnostics}
...
class DiagnosticsStorageTest(unittest.TestCase):
    ...
    def test_receive_diagnostics(self):
        ...
        ui.update.assert_called_with(test_file_path, "test_server",
                                     {test_file_path: {
                                         'test_server': [minimal_diagnostic]
                                     }})
