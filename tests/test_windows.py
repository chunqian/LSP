...
def mock_start_session(window: MockWindow, workspace_folders: 'List[WorkspaceFolder]',
                       designated_folder: 'Optional[WorkspaceFolder]', config: ClientConfig,
    ...
    return create_session(config=TEST_CONFIG,
...
class WindowRegistryTests(unittest.TestCase):
    def test_can_get_window_state(self):
        windows = WindowRegistry(TestGlobalConfigs(), TestDocumentHandlerFactory(), mock_start_session, test_sublime,
...
class WindowManagerTests(unittest.TestCase):
    def make(self, *args: 'Any',
             **kwargs: 'Any') -> 'Tuple[MockWindow, MockDocuments, MockHandlerDispatcher, WindowManager]':
        ...
        wm = WindowManager(window=window,
                           workspace=workspace,
                           settings=MockSettings(),
                           configs=MockConfigs(),
                           documents=docs,
