...
def read_client_config(name: str, client_config: Dict) -> ClientConfig:
    ...
    return ClientConfig(name, client_config.get("command", []), client_config.get("tcp_port", None),
                        client_config.get("scopes", []), client_config.get("syntaxes", []),
                        client_config.get("languageId", ""), languages, client_config.get("enabled", False),
                        client_config.get("initializationOptions", dict()), client_config.get("settings", dict()),
                        client_config.get("env", dict()), client_config.get("tcp_host", None),
...
def update_client_config(config: ClientConfig, settings: dict) -> ClientConfig:
    ...
    return ClientConfig(config.name, settings.get("command", config.binary_args),
                        settings.get("tcp_port", config.tcp_port), settings.get("scopes", default_language.scopes),
                        settings.get("syntaxes", default_language.syntaxes),
                        settings.get("languageId", default_language.id),
                        read_language_configs(settings) or config.languages, settings.get("enabled", config.enabled),
                        settings.get("init_options", config.init_options), settings.get("settings", config.settings),
                        settings.get("env", config.env), settings.get("tcp_host", config.tcp_host),
