...
class WindowManager(object):
    ...
    def _disable_temporarily(self, session: Union[str, Session], ex: Exception,
        ...
        if isinstance(session, Session):
            ...
            msg = "\n\n".join(["Could not start {}", "{}",
                               "Server will be disabled for this window"]).format(name, str(ex))
    ...
    def _handle_post_initialize(self, session: Session, error: Optional[Dict[str, Any]]) -> None:
        ...
        session.on_request("workspace/applyEdit",
                           lambda params, request_id: self._apply_workspace_edit(params, session.client, request_id))

        session.on_notification("textDocument/publishDiagnostics",
...
class WindowRegistry(object):
    def __init__(self, configs: GlobalConfigs, documents: Any, session_starter: Callable, sublime: Any,
