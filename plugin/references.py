...
class LspSymbolReferencesCommand(LspTextCommand):
    ...
    def show_quick_panel(self, references_by_file: Dict[str, List[Tuple[Point, str]]]) -> None:
        ...
        window = self.view.window()
        if window:
            window.show_quick_panel(self.reflist, self.on_ref_choice, flags, selected_index, self.on_ref_highlight)
    ...
    def _group_references_by_file(self, references: List[ReferenceDict]) -> Dict[str, List[Tuple[Point, str]]]:
