...
class LspGotoCommand(LspTextCommand):
    ...
    def handle_response(self, response: Optional[Any], position: int) -> None:
        ...
        def open_location(window: sublime.Window, location: Tuple[str, str, Tuple[int, int]]) -> None:
            ...
            window.open_file(file_path_and_row_col, sublime.ENCODED_POSITION)

        def select_entry(window: sublime.Window, locations: List[Tuple[str, str, Tuple[int, int]]], idx: int,
        ...
        def highlight_entry(window: sublime.Window, locations: List[Tuple[str, str, Tuple[int, int]]],
                            idx: int) -> None:
        ...
        if response:
            ...
            if len(locations) == 1:
                ...
                window.show_quick_panel(items=[display_name for file_path, display_name, rowcol in locations],
