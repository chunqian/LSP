...
from .core.logging import printf, set_message_logging, get_message_logging
...
class SignatureHelpListener(LSPViewEventListener):
    ...
    def initialize(self) -> None:
        ...
        if session:
            signatureHelpProvider = session.get_capability('signatureHelpProvider')
            if signatureHelpProvider:
                self._signature_help_triggers = signatureHelpProvider.get('triggerCharacters')
    ...
    def request_signature_help(self, point: int) -> None:
        ...
        if client:
            ...
            client.send_request(Request.signatureHelp(document_position),
    ...
    def handle_response(self, response: Optional[Dict], point: int) -> None:
        ...
        if self.view.sel()[0].begin() == self.requested_position:
            self._help = create_signature_help(response)
            if self._help:
                content = self._help.build_basic_popup_content(self._renderer)
    ...
    def _print(self, content: str) -> None:
        ...
        if get_message_logging() != content:
            printf(content)
            set_message_logging(content)
