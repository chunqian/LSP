...
def request_code_actions(view: sublime.View, point: int, actions_handler: Callable[[CodeActionsByConfigName],
                                                                                   None]) -> CodeActionsAtLocation:
    diagnostics_by_config = filter_by_point(view_diagnostics(view), Point(*view.rowcol(point)))
...
def request_code_actions_with_diagnostics(
        view: sublime.View, diagnostics_by_config: Dict[str, List[Diagnostic]], point: int,
        ...
        if session.has_capability('codeActionProvider'):
            if session.config.name in diagnostics_by_config:
                ...
                if file_name:
                    ...
                    if session.client:
                        session.client.send_request(Request.codeAction(params),
...
def execute_server_command(view: sublime.View, config_name: str, command: Mapping[str, Any]) -> None:
    ...
    if client:
        client.send_request(Request.executeCommand(command), handle_command_response)
